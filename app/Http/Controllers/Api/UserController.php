...
class UserController extends BaseController
{
    ...
    public function ailmentsList(Request $request){
        try {
            if(isset($request->language_code) && !empty($request->language_code)){
                ...
                $ailmentsList = $ailments->map(function ($ailment) use ($request){
                    return [
                        'id' => $ailment->id,
    ...
    public function fatchUserDataOnUid(Request $request){
        ...
        if(isset($uniqueId)){
            $fatchUserDataOnUid  = getUserData($uniqueId);
            if(isset($fatchUserDataOnUid)){
                $data = $this->userResponse($fatchUserDataOnUid);
                return $this->sendResponse($data,"User Data Received Successfully",true);
            }else{
                return $this->sendResponse(null,'Unique Id Wrong!',false);
            }
        }
        return $this->sendResponse(null,'UniqueId Required!',false);
    }
